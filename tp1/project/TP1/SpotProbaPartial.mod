...
// troisième partie : 
maximize sum(ima in Images : TY[ima] == 1) (sum(ins in Instruments) (PA[ima] * assignedTo[ima, ins] * (1 - Failure[ins]) * (1 - ProbaSup[ima]))) + sum(ima in Images : TY[ima] == 2) (selection[ima] * PA[ima] * (1 - Failure[1]) * (1 - Failure[3]) * (1 - ProbaSup[ima]));
